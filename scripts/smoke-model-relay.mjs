...
const userSignature = process.env.MODEL_RELAY_USER_SIGNING_KEY ? crypto.createHmac('sha256', process.env.MODEL_RELAY_USER_SIGNING_KEY).update(userId).digest('hex') : '';
const userHeaders = { authorization: `Bearer ${internal}`, 'x-user-id': userId, ...(userSignature ? { 'x-user-signature': userSignature } : {}) };
...
const models = await request('/v1/models', { headers: userHeaders });
...
if (process.env.MODEL_RELAY_SMOKE_GENERATE === 'true') {
  ...
  const accepted = await request('/v1/generations', { method: 'POST', headers: userHeaders, body: JSON.stringify({ modelId: model.modelId, contentType: model.type, prompt: 'smoke test', idempotencyKey: `smoke-${Date.now()}` }) });
  const snapshot = await request(`/v1/tasks/${accepted.taskId}`, { headers: userHeaders });
